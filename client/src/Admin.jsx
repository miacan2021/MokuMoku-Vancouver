...
const Admin = () => {
  ...
  return (
    ...
    <Box sx={{width:'80vw', maxWidth: 1100, m:'auto', overflow:'hedden'}} display="flex"
     justifyContent="center" alignItems="center" flexDirection='column'
     >
  {!inPage ? 
   <Box sx={{width:'300px', height:'200px', boxShadow: 2,
   background:'#fffffe', borderRadius:'5px', m:3, p:2}} display="flex"
   justifyContent="space-around" alignItems="center" flexDirection='column'
   >
    {err &&
    <Alert severity="error" sx={{p: 1, m: 1, width:'100%'}}>Error! Please make sure your username and password.</Alert>
    }
    ...
    {events.length !== 0 &&
      <Grid container spacing={0.5} sx={{m:'auto', width:'100%'}} alignItems='center' justifyContent='center'>
          {events.map(event => (
      ...
      {inPage && <Button variant="contained" onClick={logout} sx={{mr: 1}} >Logout</Button>}
